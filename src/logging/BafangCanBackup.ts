    ...
    public static saveBackup(
        ...
        if (display && display.available) {
            backup_obj.display = {
                ...
                data1: display.bafangMileageData,
                data2: display.bafangSpeedAndServiceData,
        ...
        const backup_text = JSON.stringify(backup_obj);
        const dir = path.join(getAppDataPath('open-bafang-tool'), `backups`);
        try {
